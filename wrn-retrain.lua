...
function train()
  ...
  local indices = torch.randperm(provider_val.trainData.data:size(1)):long():split(math.floor(opt.batchSize))
  local indices1 = torch.randperm(provider.data:size(1)):long()
  ...
  for t,v in ipairs(indices) do
    ...
    alpha = math.floor(((iteration-0)/50)*3*opt.batchSize)
    iteration = iteration + 1 
    xlua.progress(t,#indices)
    local current_addition = indices1:narrow(1,ptr,alpha)
    ptr = math.min(ptr+alpha,indices1:size(1))
    local Xex =  provider.data:index(1,current_addition)
    local Yex =  provider.labels:index(1,current_addition)
    local inputs = provider_val.trainData.data:index(1,v):cat(Xex,1)
    local targets = torch.CudaTensor(opt.batchsize+alpha)
    targets:copy(provider_val.trainData.labels:long():index(1,v):cat(Yex:long(),1))
    local op_ = model:forward(inputs)
    local pred 
    _,pred = torch.max(op_,2)
    ...
    targets[{{targets:size(1)-alpha+1,targets:size(1)}}] = pred:cuda()
  end
